...
hence $H_n\left(f^{-1}(B_\alpha(\varepsilon))\cap f^{-1}(S_\alpha^+(\theta,\varepsilon))\right)$ has rank $\mu$. \textbf{Mostra che Lefschetz thimbles sono lin indip.} 
...
\end{align*}
Moreover the coeficients $n_\alpha$ can be computed via intersection theory, as a consequence of Pham's duality result ($\S 5$ \cite{pham}). 
