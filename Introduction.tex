...
\color{magenta}
Let $\theta=0$, locally in a neighbourhood of a critcal point $x_\alpha$ we can choose a coordinate system $x_1,...,x_n$ such that $f(x)-f(x_\alpha)=x_1^2+...+x_n^2$. Then $\mathrm{Re}(f)$ is a Morse function with simple, isolated and non degenerate critical points. We choose a Riemannian metric $g$ on $X$ and we define the Lefschetz thimbles $\Gamma_{\alpha,z}$ as the steepstet descendet path issuing from a critcal point $x_\alpha$ (see \cite{pham}\cite{Witten}): denote $x'=\mathrm{Re}(x)$ and $x''=\mathrm{Im}(x)$, then 
\begin{equation}
