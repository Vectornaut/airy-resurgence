...
\begin{equation}
\hat{P}_j:=P(-\alpha_j-\zeta_j)+\partial_{\zeta,\alpha_j}^{-1}\circ Q(-\alpha_j-\zeta_j)+\partial_{\zeta,\alpha_j}^{-2}\circ\sum_{r=0}^dR_r(\partial_{\zeta,\alpha_j}^{-1})\circ (-\alpha_j-\zeta_j)^r
\end{equation}
...
\textcolor{DarkCyan}{[Instead of thinking of $z$ this way, maybe we should think of it as the coordinate on the ``generic cotangent fiber'' over a non-singular point?]} The translation structure also gives us an isomorphism $z \maps T^*B_b \to \C$ when $b \in B$ is an ordinary point, and an isomorphism $z \maps T^*B_b^{\otimes n} \to \C$ when $b$ is a branch point of cone angle $2\pi n$. At an ordinary point, we can define $z$ simply as the map
\begin{align*}
z \maps T^*B_b & \to \C \\
...
The asymptotics of $f$ at the starting point of $\Gamma_{\zeta, \alpha}$ control the asymptotics of $\laplace_{\zeta, \alpha} f$ at the infinite end of $\Gamma_{z, 0}$. Once we see how this works for $\alpha = 0$, Section~\ref{translation} will do the rest. Let $F = \laplace_{\zeta, 0} f$. Equation~1.8 of \cite{laplace-tfm} shows\footnote{The argument cited still works in our generality. For holomorphic $f$, one can also use Equation 1.5 of {\em Borel-Laplace Transform and Asymptotic Theory} \textcolor{magenta}{(Sternin \& Shatalov)}.} that
...
has an infinite radius of convergence, then the Taylor coefficients decay fast enough for us to take the Laplace transform term by term~\cite[Theorem 5.20]{MS}:
\begin{align*}
...
\color{black}
\subsubsection{Action on transseries}
...
as the formal inverse of $\laplace_{\zeta_\alpha}$ on the cotangent fibre over $\alpha$. Explicitly, from identity \eqref{change-chart}
 ...
 \textcolor{magenta}{I think we should add a remark, saying that on $B$ there is no special coordinate, and this reflects on the fact that $\borel$ is defined equivalently for every choice of coordinates.}
...
\subsection{Properties of the Borel transform}
...
The space of Gevrey-$1$ formal series is a $\C$-algebra with unit $1$, thus we may wonder if $\borel$ is an algebra isomorphism between $\C\llbracket z^{-1}\rrbracket_1$ and $\C\lbrace\zeta\rbrace$. The answer is yes, once we endow $\C\lbrace\zeta\rbrace$ with the convolution product and with the formal unit $\delta$. Let $\tilde{\Phi}, \tilde{\Psi}\in \C \llbracket z^{-1} \rrbracket_1$ (where we assume they are series on the fiber over $0$), then 
\[\borel_\zeta(\tilde{\Psi}\tilde{\Phi})=:\tilde{\psi}\ast\tilde{\phi}\]

and $\delta:=\borel 1$. In addition, it follows that the convolution product $\ast$ can be explicitly characterized as 

\begin{equation}
    \tilde{\psi}\ast\Tilde{\phi}=\int_0^{\zeta}d\zeta' \tilde{\psi}(\zeta-\zeta')\tilde{\phi}(\zeta')
\end{equation}

the proof is a simple computation, and we refer to Definition 5.12 and Lemma 5.14 \cite{MS16}.
...
\begin{itemize}
\item $P$ is a monic degree-$d$ polynomial with simple zeros at $-\alpha_j$ for $j=1,...,d$ \textcolor{orange}{[} i.e such that $P'(-\alpha_j)\neq 0$ for every $j=1,...,d$;
...
\begin{align*}
&\partial_{\zeta,\alpha_j}^{-2}\circ\left[R_0(\partial_{\zeta,\alpha_j}^{-1})+R_1(\partial_{\zeta,\alpha_j}^{-1})\circ (-\zeta)+...+R_d(\partial_{\zeta,\alpha_j}^{-1})\circ (-\zeta)^d\right]=\sum_{\lambda\leq -2} \partial_{\zeta,\alpha_j}^\lambda\circ r_\lambda \\
&\text{ with }r_\lambda=\sum_{k=0}^d R_{k,\lambda}(-\zeta)^k.
\end{align*}
...
\begin{lemma}
 Let $\hat{\psi}_j$ be a solution of $\hat{P}_j\hat{\psi}_j=0$ for every $j=1,...,d$ and $\hat{P}_j$ defined by \eqref{P-hat_j}. If $\hat{\psi}_j$ admits a Laplace transform $\Psi_j=\laplace_{\zeta,\alpha_j}^{\theta_j}\hat{\psi}_j$ for $j=1,...,d$ and for a suitable direction $\theta_j$, then $\mathcal{P}\Psi_j=0$.   
\end{lemma}

\begin{proof}
    \begin{align*}
        \mathcal{P}\Psi_j&=\mathcal{P}\laplace_{\zeta,\alpha_j}^{\theta_j}\hat{\psi}_j\\
        &=\laplace_{\zeta,\alpha_j}^{\theta_j}\left[P(-\zeta)+\fracderiv{-1}{\zeta}{\alpha}\circ Q(-\zeta)+\fracderiv{-2}{\zeta}{\alpha}\circ\sum_{r=0}^dR_r(\fracderiv{-1}{\zeta}{\alpha})(-\zeta)^r\right]\hat{\psi}_j\\
        &=\laplace_{\zeta,\alpha_j}^{\theta_j}\left[P(-\zeta_j-\alpha_j)+\fracderiv{-1}{\zeta}{\alpha}\circ Q(-\zeta_j-\alpha_j)+\fracderiv{-2}{\zeta}{\alpha}\circ\sum_{r=0}^dR_r(\fracderiv{-1}{\zeta}{\alpha})(-\zeta_j-\alpha_j)^r\right]\hat{\psi}_j\\
...
\begin{align*}
  \borel_{\zeta}\left(\mathcal{P}\series{\Psi}_j\right)&= \left[P(-\zeta)+1\ast_{\alpha_j} Q(-\zeta)+\zeta \ast_{\alpha_j} \left(\sum_{r=0}^{d}\borel_\zeta(R_r(z^{-1}))\ast_{\alpha_j}(-\zeta)^r\right)\right]\series{\psi}_j(\zeta_j)\\
  &=\left[P(-\zeta)+\partial_{\zeta,\alpha_j}^{-1}\circ Q(-\zeta)+\fracderiv{-1}{\zeta}{\alpha_j} \left(\sum_{r=0}^{d}R_r(\fracderiv{-1}{\zeta}{\alpha_j} )(-\zeta)^r\right)\right]\series{\psi}_j(\zeta_j)\\&=\hat{P}_j\tilde{\psi}_j(\zeta_j)
\end{align*}
%\begin{multline*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item along that direction, the germ $\tilde{\psi}_j$ grows sub-exponentially. 
    \end{itemize}
    The singularities of $\hat{P}_j$ are at $P(-\alpha_j-\zeta_j)=0$, hence at $\zeta_j=\alpha_k-\alpha_j$ for every $k=1,...,d$. \textcolor{magenta}{[why they are there? add comment][Take a more careful look at Lemma 3.9 \cite{Del}]}
\end{proof}
...
Let $X$ be a $N$-dimensional manifold equipped with a volume form $\nu\in\Gamma(X,\Omega^N)$ and a holomorphic Morse function $f\colon X\to\C$ with only simple critical points. Set
\begin{equation}\label{exp-int}
I(z)\defeq\int_{\mathcal{C}}e^{-zf}\nu
\end{equation}
where $\mathcal{C}$ is a contour chosen to make the integral well-defined.  
At a Morse critical point $x_\alpha$ of $f$, the saddle point approximation gives the formal series 
\begin{equation}
...
\begin{enumerate}
\item\label{int:series-gevrey} The series $\tilde{\varphi}_\alpha$ is Gevrey-1, thus $\hat{\varphi}_\alpha(\zeta)\defeq\borel_\zeta(\tilde{\varphi}_\alpha)$ converges near $\zeta=\zeta_{\alpha}$
...
\textcolor{magenta}{[In the proof we use $\zeta_\alpha=f(x_\alpha)$; however this is in contrast with our notation as $\zeta_\alpha=\zeta-f(x_\alpha)$. Should we change it?]}
\begin{proof}
...
as $z \to \infty$. [Veronica: I don't think this is the claim of steepest descendet method]} 
...
\begin{equation}
    e^{-zf(x_\alpha)}I_\alpha(z)\approx\sqrt{2\pi} \sum_{n \ge 0} (2n-1)!!\,b_{2n}^\alpha\,\left[z^{-(n+1/2)} \operatorname{erf}\big(\varepsilon \sqrt{z/2}\big)-2e^{-z\varepsilon^2/2}\sum_{\substack{k\in\mathbb{N}_+\\k\leq n}} \frac{\varepsilon^{2k-1}}{(2k-1)!!}z^{n-k+1}\right]
\end{equation}
hence we have to estimate the growth of 
\[-2\sqrt{2\pi} e^{-z\varepsilon^2/2} \sum_{n \ge 0} (2n-1)!!\,b_{2n}^\alpha \sum_{\substack{k\in\mathbb{N}_+\\k\leq n}} \frac{\varepsilon^{2k-1}}{(2k-1)!!}z^{n-k+1}\]
...
Call the right-hand side $\tilde{I}_\alpha$. We now see that $a_{\alpha,n} = (2n-1)!!\,b_{2n}^\alpha$ in the statement of the theorem. \textbf{[Resolve discrepancy with previous calculation.]} Note that 
\begin{align*}
