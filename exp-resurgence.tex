...
Under these assumptions, $\mathcal{P}\Phi = 0$ admits a basis of formal solutions \cite{int-irreg}\cite[Proposition~2.2.7, p.~111]{EcalleIII}: let $\tau_j:=Q(-\alpha_j)/P'(-\alpha_j)\in\mathbb{Q}^\times$, then the formal solutions $\series{\Psi}_1,...,\series{\Psi}_d$ are of the form 
\begin{equation}
...
However, can we find a distinguished basis for solutions in a purely analytic way? This should be possible as the coefficients of the ODE are regular enough. In fact, the Main Asymptotic Existence Theorem (M.A.E.T.) guarantees the existence of an analytic frame of solutions asymptotics to a formal frame \textcolor{magenta}{[reference-see Sauzin-Mitchi or Delabere book]}, but it is not a constructive method (see Balser chap. 14). Conversely, we can explicitly build a distinguished basis of analytic solutions: 
...
For every $j=1,...,d$ the solution $\psi_j(\zeta_j)$ of Theorem \ref{thm1} admits a well defined Laplace transform $\Psi_j:=\mathcal{L}_{\zeta_j}^{\theta_j}$ for some direction $\theta_j$.
...
%\color{magenta}
%
%where the explicit definition of $\mathcal{L}_{\zeta_{\alpha},0}^{\theta}\varphi$ is
%
%\begin{equation}
%\mathcal{L}_{\zeta_\alpha,0}^{\theta}\varphi:=\int_{\Gamma_{\zeta_\alpha,0}^{\theta}}e^{-z\zeta_\alpha}\varphi(\zeta_\alpha-\alpha)d\zeta_\alpha=\int_{\Gamma_{\zeta_\alpha,0}^{\theta}}e^{-z\zeta_\alpha}\varphi(\zeta)d\zeta
%\end{equation}
%
%\color{black}
...
For every $j=1,...,d$ let $\psi_j(\zeta_j)$ be the unique solution of $\hat{P}_j\psi_j=0$ as in in Theorem \ref{thm1-dim}. Then $\psi_j(\zeta_j)$ admits a well defined Laplace transform $\Psi_j:=\mathcal{L}_{\zeta_j}^{\theta_j}$ for some direction $\theta_j$.
...
\item[$(b)$] let $\hat{\psi}_j$ be the sum of the germ $\series{\psi}_j:=\mathcal{B}\series{\Psi}_j$; prove that $\hat{\psi}_j \propto \psi_j$ 
\item[$(c)$] prove that $\mathcal{L}_{\zeta_j}^{\theta_j}\circ\mathcal{B}\series{\Psi}_j\propto \Psi_j $. 
\end{itemize}
...
\begin{multline*}
\mathcal{B}\left(\mathcal{P}\series{\Psi}_j\right)=\mathcal{B}\left(\mathcal{P}_j\sum_{n=0}^{\infty}a_{j,n}z^{-n-\tau_j}\right)=\\
...
%
%\begin{align*}
%\series{\psi}_j(\zeta_j):=\mathcal{B}\series{\Psi}_j(\zeta_j)&=\mathcal{B}\left(e^{-\alpha_j z}z^{-\tau_j}\sum_{n=0}^{\infty}a_{j,n}z^{-n}\right)(\zeta_j)=\mathcal{B}\left(z^{-\tau_j}\sum_{n=0}^{\infty}a_{j,n}z^{-n}\right)(\zeta-\alpha_j)\\
