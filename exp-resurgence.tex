...
The {\em asymptotic expansion} map $\text{\ae}^\theta$ sends a holomorphic function that vanishes at $\infty$ to the unique power series of the form \eqref{series:asymp-defn} which is asymptotic to it on the half-plane $\Re e^{i\theta}z > 0$ \textcolor{orange}{[Theorem~C.11, Nikolaev, ``Existence and uniqueness\ldots'']}.
\end{defn}
...
\item By construction, $\text{\ae}^\theta \hat{\Phi} = \series{\Phi}$. It turns out that $\series{\Phi}$ is not only asymptotic to $\hat{\Phi}$, but {\em {uniformly} $1$-Gevrey}-asymptotic~\cite[Corollary~5.23]{diverg-resurg-i}.
\end{enumerate}
...
\begin{tikzcd}
& \text{problem} & \\
\hat{\Phi} \arrow[ru, dotted, no head, tail] \arrow[rr, mapsto, "\text{\ae}^\theta", "\text{($1$-Gevrey)}"'] & & \series{\Phi} \arrow[lu, no head, tail, "\parbox{10mm}{\centering\scriptsize formally solves}"'] \arrow[dd, mapsto, "\borel"] \\
& & \\
...
\begin{tikzcd}
& \text{problem} & \\
\Phi \arrow[ru, no head, tail, swap, "\text{solves}"' ] \arrow[rrd, mapsto, "\text{\ae}^{\theta}"]& & \\
\hat{\Phi} \arrow[rr, "\text{\ae}^{\theta}", "\text{($1$-Gevrey)}"] & & \series{\Phi} \arrow[luu, no head, tail, swap, "\parbox{10mm}{\centering\scriptsize formally solves}"] \arrow[dd, mapsto, "\borel"] \\
& & \\
...
\begin{enumerate}
\item Take the asymptotic expansion $\series{\Phi} = \text{\ae}^{-\theta} \Phi$.
...
\end{tikzpicture}
\end{center}
When the half-plane extends along the $-\theta$ direction, the sum $\hat{f}$ of the Borel transform of $\text{\ae}^{-\theta} F$ has an absolutely convergent Laplace transform along the $\theta$ direction. In fact, we have $|\hat{f}| \lesssim e^{c |\zeta|}$ uniformly over all $\zeta$ in a constant-radius neighborhood of the ray $e^{i\theta}[0, \infty)$. 
...
\begin{equation}
\begin{tikzcd}
I_{\alpha}(z) \arrow[r,"\text{\ae}^\theta"] & \tilde{I}_{\alpha}(z)\arrow[dd,"\borel"]\\
& \\
...
The Borel transform $\borel\textcolor{orange}{_\zeta}$ turns formal power series in $z^{-1}$---which represent functions defined all non-singular cotangent spaces---into formal power series in $\zeta$. More precisely,   $\borel_\zeta$ is a $\C$-linear map 
  \begin{align*}
...
Let $\varphi=\sum_{n=0}^{\infty}a_n\zeta^n$, then from identity \eqref{change-chart}
  ...
  \begin{align*}
      \borel_\zeta\laplace_{\zeta,\alpha}\varphi=\borel_\zeta\left[e^{-\alpha z
      }\laplace_{\zeta_\alpha,0}\varphi\right]
  \end{align*}
We compute the left-hand-side
  ...
  \begin{align*}
      ...
      &=\borel_\zeta\left[
      \sum_{n=0}^{\infty}a_n\int_{0}^{\infty} e^{-\zeta z} \zeta^n d\zeta\right]\\
      &=\borel_\zeta\left[
      \sum_{n=0}^{\infty}a_n z^{-n-1}n! \right]\\
...
Then, comparing term by term in powers of $\zeta$ in the second identity we get 
  \begin{align*}
...
\subsubsection{Change of translation chart} 

Following Section \ref{translation}, we introduce $\borel_{\zeta_\alpha}$
as the formal inverse of $\laplace_{\zeta_\alpha}$ on the cotangent fibre over $0$. Explicitly, from identity \eqref{change-chart}
  \begin{align*}
...
\item[(vi)] if $\tilde{\Psi}\in z^{-1}\C \llbracket z^{-1} \rrbracket$, then $\borel_\zeta(\tilde{\Phi}\tilde{\Psi})=\int_0^{\zeta}d\zeta' \tilde{\phi}(\zeta-\zeta')\tilde{\psi}(\zeta')=:\tilde{\phi}\ast \tilde{\psi}$, where $\tilde{\psi}=\borel_{\zeta}\tilde{\Psi}$.
\end{itemize} 
...
\left(\tfrac{\partial}{\partial \zeta}\right)^{n+1}\,\partial^{\alpha-1}_{\zeta } \left[ \borel \left(z^{-(k+1)}\right)(\zeta) \right] = \frac{\zeta^{k-(n+\alpha)}}{\Gamma\big(k-(n+\alpha)+1\big)}
\end{equation}
