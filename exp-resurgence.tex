...
\subsubsection{Lifting to a countable cover}
...
\end{align}
where $\Omega$ is a path that comes from $\infty$ along $-i \pi + (0, \infty)$ and goes to $\infty$ along $i \pi + (0, \infty)$. The integral converges when $z$ is in the right half-plane. We get formula~\eqref{integral:mod-bessel-lifted} when we choose a rational parameter $\nu = m/n$.

When $\nu$ goes to $0$, formula~\eqref{integral:mod-bessel-lifted} becomes
...
\left[ \big(\tfrac{\partial}{\partial \zeta}\big)^{n-1} \circ\big[ \zeta^{n-1} - 1 \big] - c_n^{(1)} \partial_{\zeta,\alpha_k}^{n-2}\circ \zeta^{n-2} - c_n^{(2)} \partial_{\zeta,\alpha_k}^{n-3} \circ\zeta^{n-3} - \ldots - c_n^{(n-1)} \right] v = 0.
\end{equation}

Using the Liebniz rule for the differential, we get 

\begin{itemize}
\item $\big(\tfrac{\partial}{\partial \zeta}\big)^{n-1} \circ\big[ \zeta^{n-1} - 1 \big]=\sum_{j=0}^{n-1} (n-1 j) v^{(n-j-1)}\big(\tfrac{\partial}{\partial \zeta}\big)^{j} \big[ \zeta^{n-1} - 1 \big]=(\zeta^{n-1} - 1)\partial_{\zeta}^{n-1}v+ (n-1)^2\zeta^{n-2} \partial_\zeta^{n-2}v +...+(n-1)! v $
\end{itemize}
