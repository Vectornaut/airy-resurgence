...
\end{theorem}
\begin{proof}
It is enough to show that $\hat{P}_j$ satisfies the assumptions of Theorem 2.1.3 of \textcolor{orange}{\texttt{reg-sing-volterra}}.  In fact, $P(-\zeta_j-\alpha_j)=P(-\zeta)$ is a polynomial also defined at $\zeta=\alpha_j$ and by assumption $P'(-\alpha_j)\neq 0$. $Q$ is also a polynomial defined at $\zeta=\alpha_j$. Finally, assuming $R_k(z)=\sum_{j\geq 0} R_{k,j}z^j$ 
...
where $\Lambda_j$ is a thimble in $H_{1}^{B,z}$ through a critical point of $f$. We can distinguish between the case when $f$ has two distinct critical points $x_+, x_-$ and only one critical point $x$. In both cases, we will compute the differential equation satisfied by $I$. 
...
When $p=0$, the critical point of $f$ is $x=0$ and we can argue as before: $\laplace_{\zeta,\alpha}^{\theta}v$ is a solution of \eqref{eqn:degree-3-poly-1root} if and only if $v$ is a solution of 
