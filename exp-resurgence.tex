...
over all orders $n$ and all $z \in \Omega$. We'll use $\lesssim$ throughout the paper to mean ``bounded by a constant multiple of.''
\end{defn}
...
\begin{tikzcd}
& \text{problem} & \\
\hat{\Phi} \arrow[ru, dotted, no head, tail] \arrow[rr, mapsto, "\aexp^\theta", "\text{($1$-Gevrey)}"'] & & \series{\Phi} \arrow[lu, no head, tail, "\parbox{10mm}{\centering\scriptsize formally solves}"'] \arrow[dd, mapsto, "\mathcal{B}"] \\
& & \\
...
\hat{\Phi} \arrow[rr, "\aexp^{\theta}", "\text{($1$-Gevrey)}"'] & & \series{\Phi} \arrow[luu, no head, tail, swap, "\parbox{10mm}{\centering\scriptsize formally solves}"] \arrow[dd, mapsto, "\mathcal{B}"] \\
& & \\
...
Borel regular functions can be characterized as functions that are approximated well, asymptotically, by polynomials. Watson showed a century ago \textcolor{orange}{[Watson]} that a function $F$ that vanishes at $\infty$ is Borel regular whenever there's an obtuse-angled sector around $\infty$ and a pair of constants $C, A \in (0, \infty)$ for which
\[ \left|F - \left(\frac{a_1}{z} + \frac{a_2}{z^2} + \ldots + \frac{a_{n-1}}{z^{n-1}} \right) \right| \le \frac{C A^n n!}{|z|^n} \]
over all orders $n$ and all $z$ in the sector.

Nevanlinna's improvement of Watson's theorem (Sokal, ``An improvement of Watson's theorem on Borel summability'', see a proof in Nikita, ``Exact Solutions for the Singularly Perturbed Riccati Equation and Exact WKB Analysis'' Theorem~B.15) tells us that a function $\Phi$ holomorphic in a sectorial neighbourhood $\mathsf{A}_\theta:=\{z\in\C \vert \Re(e^{i\theta}z)\}$ is Borel regular. Indeed let $\series{\Phi}\in\C\llbracket z^{-1}\rrbracket$ be the ``Gevrey-asymptotics'' of $\Phi$ as $\Re(e^{i\theta}z)\to \infty$, then its Borel transform $\hat{\phi}\defeq\mathcal{B}\series{\Phi}$ is a germ of holomorphic function at the origin and it can be analytically continued in a tubular neighbourhood of the ray $e^{i\theta}[0,\infty)$ with at most exponential growth at infinity. Hence $\Phi$ is Borel-regularizable. In addition, $\series{\phi}$ converges uniformly to a holomorphic function $\phi(\zeta)$ whose Laplace transform coincides with $\Phi(z)$. In particular, we learn that being ``Gevrey-asymptotic'' to a formal series is a sufficient condition for Borel regularity. 
\subsubsection{Borel regularity sometimes explains why Borel summation works}
...
\begin{enumerate}
\item Solving a linear ODE as the one in example Equation~\eqref{eqn:bessel-rescaled-1}. The general example is the equation $\mathcal{P} \Phi = 0$ for a differential operator of the form
...
The form of $\mathcal{P}$ derives from Equation~2.2.3 \textcolor{orange}{(p.~105)} of \cite{EcalleIII}, which encompasses both linear and nonlinear ODEs of Poincar\'{e} rank~$1$. Borel summation still works well for the nonlinear ones, raising the question of whether our analysis generalizes.
...
Second of all, we want to explain why it is useful to work in the Borel plane (the spatial domain): on the hand integral equations are more regular than differential equations. On the other hand, a thimble integral in the frequency domain can be recast as the Laplace transform of a function in the spatial domain. 

\subsubsection{Why does Borel resummation work for solutions of irregular singular ODEs?}

We restrict to ODEs with irregular singularity at $\infty$ \textcolor{magenta}{and of Poincar\'e rank $1$}: 
...
\end{equation}
where $P(\lambda)$ is a (monic) degree $d$ polynomial, $Q(\lambda)$ is a degree $d-1$ polynomial and $R_0, \ldots, R_d$ are holomorphic functions on $\C$ \footnote{Looking at the existence theorem in {\tt airy-resurgence} 2.1.3, we could apply this reasoning on the analytic side for more general equations, but this particular case makes it easier to talk about the formal side as well.]}. Furthermore we assume $P$ has simple zeros $P(-\alpha_j)=0$, $j=1,...,d$ and $Q(-\alpha_j)\neq 0$.

Under these assumptions, \eqref{eqn:standard ODE} admits a basis of formal solutions \cite{int-irreg} \cite[Proposition~2.2.7, p.~111]{EcalleIII}: let $\tau_j:=Q(-\alpha_j)/P'(-\alpha_j)\in\mathbb{Q}^*$, then the formal solutions $\tilde{\Psi}_1,...,\tilde{\Psi}_d$ are of the form 
\begin{equation}
...
Classically it has been proved that the formal solutions $\series{\Psi}_j$ \eqref{formal solution} are Borel-Laplace summable and their Borel-Laplace sum $\hat{\Psi}_j$ satisfies the original equation \eqref{eqn:standard ODE} \cite{Ramis?}\textcolor{magenta}{[Check references]}. Therefore, the distinguished frame of formal solutions become a distinguished frame of analytic solutions $\hat{\Psi}_1,...,\hat{\Psi}_d$.       

However, can we find a distinguished basis of solutions in a purely analytic way? This should be possible as the coefficients of the ODE are regular enough. In fact, the Main Asymptotic Existence Theorem (M.A.E.T.) guarantees the existence of an analytic frame of solutions asymptotics to a formal frame \textcolor{magenta}{[reference-see Sauzin-Mitchi or Delabere book]}, but it is not a constructive method (see Balser chap. 14). Conversely, we can explicitly built a distinguished basis of analytic solutions: 

\begin{theorem}\label{thm1}
In the previous set-up, for every $\alpha_j$ there exists a unique solution in the $\zeta$-plane which blows-up as $\zeta^{\tau_j-1}$ at $\alpha_j$: set $\zeta_j=\zeta-\alpha_j$,  

\[{\psi}_j(\zeta_j)=\textcolor{magenta}{\zeta_j^{\tau_j-1}}+f_j\] .
\end{theorem}
\textcolor{magenta}{[verify the exponent, is it $\tau_j-1$ or $\tau_j$? What are the conditions for $f_j$?]}
...
Let $\Psi_j\defeq\mathcal{L}_{\zeta_j}^{\theta}\psi_j$ be the Laplace transform of $\psi_j$ solutions constructed in Theorem~\ref{thm1}. The following results hold true
\begin{enumerate}
\item $\Psi_j$ is asymptotic to $\series{\Psi}_j$;
\item the Borel sum of $\series{\Psi}_j$ is the same as $\Psi_j$
\end{enumerate}
\end{theorem}
...
\Psi\arrow[rru,dotted, no head, tail] &\textcolor{red}{\hat{\Psi}}\arrow[ur,red, no head, tail] & & & \tilde{\Psi}\arrow[llu,"formal",swap, no head, tail]\arrow[dd,"\mathcal{B}"]\\
& & IE & & \\
\psi\arrow[rru, no head, tail]\arrow[uu,"\mathcal{L}"]& \textcolor{red}{\hat{\psi}}\arrow[ur, no head, tail]\arrow[rrr,"sum",mapsfrom,red]\arrow[uu,"\mathcal{L}"]& & & \tilde{\psi}\arrow[llu, no head, tail]
\end{tikzcd}
\end{center} 
...
As a consequence, by properties of the Laplace transform, $\Psi_j\defeq\mathcal{L}_{\zeta_j}^{\theta}\psi_j$ is an analytic solution of the (ODE) and it is asymptotic to $\series{\Psi}_j$. Furthermore, $\Psi_j$ is proportional to $\hat{\Psi}_j$.   

\begin{remark}
There are many ways to built a frame of solutions for these kind of ODE: formally in the $z$-plane Poincar\'e gave an explicit solution \cite{int-irreg}. Formally in the $\zeta$-plane \'Ecalle figured out how to find a frame of formal solutions of the Borel transformed ODE \textcolor{magenta}{[add reference]}. It can indeed been seen as a consequence of ``resurgence'' of the ODE. Our results show how to built a frame of solutions, analytically in the Borel plane. Furthermore, it shows we are picking the same frame: from the properties of the Laplace transform we get an analytic frame asymptotic to Poincar\'e frame $\series{\Psi}_j$. From the uniqueness result of Theorem~\ref{thm1}, we get a frame of analytic solutions equivalent to \'Ecalle's ones. 
\end{remark} 

\begin{remark}
Although, in our proof we use multi-summability to ensure that the Borel sum of the formal solution $\series{\Psi}_j$ exists and it solves the ODE, the approach of Borel regularity is different as it is based on the analytic solution in the frequency domain ($z$-plane). \textcolor{magenta}{[Is it possible to prove Borel regularity without proving Borel summability? For instance, can we argue that $\psi_j$ being the unique solution of (IE) with a certain behaviour is enough to say that it is the sum of $\series{\psi}_j$? Check carefully the argument of multi-summability]} 
\end{remark}
...
\begin{tikzcd}
& & ODE\arrow[dd,"\mathcal{B}"]\arrow[dd,"\mathcal{L}^{-1}",swap] & & \\
\Phi\arrow[rru,dotted, no head, tail] &\textcolor{red}{\hat{\Phi}}\arrow[ur,red, no head, tail] & & & \tilde{\Phi}\arrow[llu,"formal",swap, no head, tail]\arrow[dd,"\mathcal{B}"]\\
& & IE & & \\
\phi\arrow[rru, no head, tail]\arrow[uu,"\mathcal{L}"]& \textcolor{red}{\hat{\phi}}\arrow[ur, no head, tail]\arrow[rrr,"sum",mapsfrom,red]\arrow[uu,"\mathcal{L}"]& & & \tilde{\phi}\arrow[llu, no head, tail]
\end{tikzcd}
\end{center}
...
\begin{equation}
\begin{tikzcd}
I_{\alpha}(z) \arrow[r,"\ae^\theta"] & \tilde{I}_{\alpha}(z)\arrow[dd,"\mathcal{B}"]\\
& \\
\hat{\iota}_\alpha(\zeta)\arrow[uu, "\mathcal{L}^\theta_\alpha "] & \tilde{\iota}_{\alpha}(\zeta) \arrow[l, "\text{sum}"]
\end{tikzcd}
\end{equation}
\begin{itemize}
...
\begin{align*}
\borel \maps \C \llbracket z^{-1} \rrbracket & \to \C\delta \oplus \C \llbracket \zeta \rrbracket \\
1 & \mapsto \delta\vphantom{\frac{1}{1}} \\
z^{-n-1} & \mapsto \frac{\zeta^n}{n!}\qquad\text{when } n \ge 0
\end{align*}
In general, by linearity,
\begin{align*}
\mathcal{B}\big[a_0 + a_1 z^{-1} + a_2 z^{-2} + a_3 z^{-3} + a_4 z^{-4} + \ldots \big] = a_0\delta + a_1 + a_2 \zeta + a_3 \frac{\zeta^2}{2!} + a_4 \frac{\zeta^3}{3!} + \ldots.
\end{align*}
...
\[\tilde{\Phi}(z)=\sum_{n\geq 0}a_nz^{-n}\in\C \llbracket z^{-1} \rrbracket_1\]
...
\begin{lemma}
Let $\tilde{\Phi}(z)\in\C \llbracket z^{-1} \rrbracket$. Then, $\tilde{\Phi}\in\C \llbracket z^{-1} \rrbracket_1$ is Gevrey-1 if and only if $\mathcal{B}(\tilde{\Phi})\in\C\lbrace\zeta\rbrace$ is a germ of holomorphic functions. 
\end{lemma}
...
\item[(iii)] if $n\in\Z$ and $z^nf(z)\in z^{-1}\C \llbracket z^{-1} \rrbracket$, then $\mathcal{B}(z^n\tilde{\Phi}(z))=\partial_\zeta^n\tilde{\phi}(\zeta)$ 
...
\item[(vi)] if $\tilde{\Psi}(z)\in z^{-1}\C \llbracket z^{-1} \rrbracket$, then $\mathcal{B}(\tilde{\Phi}(z)\tilde{\Psi}(z))=\int_0^{\zeta}d\zeta' \tilde{\phi}(\zeta-\zeta')\tilde{\psi}(\zeta')=:\tilde{\phi}\ast \tilde{\psi}$, where $\tilde{\psi}(\zeta)=\mathcal{B}(\tilde{\Psi}(z))$.
\end{itemize} 
...
Computing
\begin{align*}
\mathcal{B}\big[ e^{-z} z^{-1/2}\,\tilde{W}_1 \big] & = \mathcal{B}\left[ \sum_{k = 0}^{\infty} \frac{\left(\tfrac{1}{2}-\tfrac{m}{n}\right)_k \left(\tfrac{1}{2}+\tfrac{m}{n}\right)_k}{k!} \left(-\frac{1}{2}\right)^k z^{-k-\frac{1}{2}} \right] \\
...
\end{align*}
we see that $\mathcal{B}\big[ e^{-z} z^{-1/2}\,\tilde{W}_1 \big]$ sums to
...
Through a similar calculation, we see that $\mathcal{B}\big[ e^z z^{-1/2}\,\tilde{W}_{-1} \big]$ sums to
...
\begin{align*}
\mathcal{B}_{\textit{phys}} \maps \C \llbracket z^{-1} \rrbracket & \to \C \llbracket \zeta \rrbracket \\
z^{-n} & \mapsto \frac{\zeta^n}{n!}.
\end{align*}
This version avoids sending $1$ to the convolution unit $\delta$, at the cost of no longer mapping multiplication to convolution or inverting the formal Laplace transform. It's sometimes convenient to $\mathcal{B}_\textit{phys}(f) = \borel(z^{-1} f)$.

For problems involving a small parameter $\hbar$ rather than a large parameter $z$, physicists also define
\begin{align*}
\mathcal{B}_{\textit{phys}} \maps \C \llbracket \hbar \rrbracket & \to \C \llbracket \zeta \rrbracket \\
