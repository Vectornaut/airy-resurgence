...
You can often find a formal power series \textcolor{magenta}{[check whether this exponent is $\tau$ on the nose or $\tau$ plus a constant]}
\[ \series{\Phi} = z^\tau \left( \frac{c_0}{z} + \frac{c_1}{z^2} + \frac{c_2}{z^3} + \frac{c_3}{z^4} + \ldots \right), \]
...
\begin{center}
\textbf{[shifted Bessel matching example above]},
\end{center}
behave near $z = \infty$, you might start by looking for ``transmonomial'' solutions $e^{\alpha z}\,\series{\Phi}$, where $\series{\Phi}$ is a formal power series of the kind above.

you might start by trying to construct a formal power series solution instead. Observing that the leading term must be a multiple of $z^{-1/2}$ and then extending term by term, you again \textcolor{magenta}{[check]} get series~\eqref{series:bessel-ex}.

%%%

For example, you might rewrite the integral
\color{DodgerBlue}
...
using the substitution $\tfrac{1}{2} \tau^2 = \tfrac{1}{3} u^3 - \tfrac{1}{4} u + \tfrac{1}{12}$. Na\"{i}vely integrating term by term, you get the series
\color{DodgerBlue}
...
\end{equation}
which has zero radius of convergence, but still makes sense as a formal power series.
\begin{itemize}
...
\item The series $\tilde{\phi}$ turns out to have a \textcolor{magenta}{[finite]} positive radius of convergence, so it defines a holomorphic function $\hat{\phi}$. By analytic continuation, we can expand the domain of $\hat{\phi}$ to a Riemann surface $B$ with a distinguished 1-form $\lambda$---the continuation of $d\zeta$.
