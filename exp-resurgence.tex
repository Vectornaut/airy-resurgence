...
\end{enumerate}
The series $\series{\Phi}$ and its Borel sum $\hat{\Phi}$ have a special relationship.
\begin{enumerate}[start=4]
\item By construction, taking the asymptotic expansion of $\hat{\Phi}$ along the half-plane $\Re e^{i\theta}z > 0$ gives back $\series{\Phi}$. In other words, $\aexp^\theta \hat{\Phi} = \series{\Phi}$, in the notation of \textcolor{orange}{[Theorem~C.11, Nikolaev, ``Existence and uniqueness\ldots'']}.

The asymptotic expansion map $\aexp^\theta$ sends each function $F$ to the unique formal power series $a_0 + a_1 z + a_2 z^2 + \ldots$ for which
\[ |F - (a_0 + a_1 z + \ldots + a_n z^n)| \in o\big(|z|^n\big), \]
as $\Re e^{i\theta} z$ grows, over all orders $n$. \textcolor{magenta}{[Wait, don't we want a series in $z^{-1}$?]}

It turns out that $\series{\Phi}$ is not only asymptotic to $\hat{\Phi}$, but {\em \textcolor{magenta}{uniformly} $1$-Gevrey}-asymptotic~\cite[Corollary~5.23]{diverg-resurg-i}: there are constants $C, A \in (0, \infty)$ for which
\[ |F - (a_0 + a_1 z + \ldots + a_{n-1} z^{n-1})| \le C A^n n!\,|z|^n, \]
on the half-plane $\Re e^{i\theta}z > 0$, over all orders $n$. \textcolor{magenta}{[I think this condition is actually \textbf{uniform} $1$-Gevrey asymptoticity on the half-plane, as defined in \cite[Definition~5.21]{diverg-resurg-i}. Note that Mitschi and Sauzin actually say ``uniformly'' in Corollary~5.23.]}
\end{enumerate}
...
We can't be sure {\em a priori} that $\hat{\Phi}$ solves our original problem. \textbf{[\ldots]} Different functions can be asymptotic to the same power series, and Borel summation picks one of them. However, in many cases, it picks correctly, producing an actual solution to your problem.
...
\textcolor{gray}{Borel regular functions have been characterized in the literature before; Watson showed a century ago \textcolor{orange}{[Watson]} that a function $\Phi$ is Borel regular whenever there's a constant $c \in (0, \infty)$ with
\[ |\Phi(z)-\sum_{k=0}^Nc_kz^{-k}| \le c^{N+1} N!|z|^{-N} \]
over all orders $N$ and all $z$ in a wide enough sector around infinity.}
...
Solving a linear ODE of Poincar\'{e} rank~$1$. One example is Equation~\eqref{eqn:bessel-rescaled-1}. The general example is the equation $\mathcal{P} \Phi = 0$ for a differential operator of the form
\[ \mathcal{P} = P(\partial_z) + \frac{1}{z} Q(\partial_z) + \frac{1}{z^2}\big[ R_0(z^{-1}) + R_1(z^{-1})\,\partial_z + \ldots + R_d(z^{-1})\,\partial_z^d \big], \]
where $P$ is a monic degree-$d$ polynomial, $Q$ is a degree-$(d-1)$ polynomial, and $R_0, \ldots, R_d$ are holomorphic functions on $\C$.
...
\color{Tan}
the first problem is solving a certain kind of ODE as \eqref{eqn:bessel-rescaled-1}. We will mainly focus on Poincar\'e rank $1$ ODEs
\begin{equation}\label{eqn:standard ODE}
\Big[ P(\partial_z)+\frac{1}{z}Q_1(\partial_z)+\sum_{j=2}^d z^{-j}R_j(\partial_z)\Big]\Psi=0
\end{equation}
where $P(\lambda)$ is a (monic) degree $d$ polynomial, $Q(\lambda)$ is a degree $d-1$ polynomial and $R_j$ is a degree $d-j$ polynomial.
\color{black}
