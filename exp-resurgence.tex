...
\begin{tikzcd}
& \text{problem} & \\
\hat{\Phi} \arrow[ru, dotted, no head, tail]\arrow[rr,red,dashed,"\text{\footnotesize Gevrey asymptotics}"'] & & \series{\Phi} \arrow[lu, no head, tail, "\parbox{15mm}{\centering\footnotesize formally solves}"'] \arrow[dd, mapsto, "\mathcal{B}"] \\
& & \\
\hat{\phi} \arrow[uu, mapsto, "\laplace_b^\theta"] & & \tilde{\phi} \arrow[ll, mapsto, "\text{sum}"] 
\end{tikzcd}
\end{center}

we read the diagram from the upper-right corner with the formal solution $\tilde{\Phi}$, then, moving clockwise, we take the Borel transform $\mathcal{B}$ which produces a germ of holomorphic function at the origin. If we can sum it and its sum $\hat{\phi}$ has a controlled behaviour along a ray going to infinity, we can take the Laplace transform $\hat{\Phi}=\mathcal{L}_b^{\theta}\hat{\phi}$ which produces a holomorphic function asymptotic to $\tilde{\Phi}$ in a given half plane. A priori it is not guaranteed that $\hat{\Phi}$ solves our original problem, as different functions can be asymptotic to the same power series, and Borel summation picks one of them. However, in many cases, it picks correctly, producing an actual solution to your problem.
...
We consider a new diagram: we start reading it from the upper-left corner, namely we assume there exists a holomorphic function $\Phi$ which is asymptotic to a formal power series $\series{\Phi}$ as $\Re e^{i\theta}z$ goes to $\infty$ in the direction of $\theta$
...
\hat{\Phi} \arrow[rr,red,dashed,"\text{\footnotesize Gevrey asymptotics}"'] & & \series{\Phi} \arrow[dd, mapsto, "\mathcal{B}"] \\
& & \\
\hat{\phi} \arrow[uu, mapsto, "\laplace_b^\theta"] & & \tilde{\phi} \arrow[ll, mapsto, "\text{sum}"] 
\end{tikzcd}
\end{center}

\textcolor{red}{[explain difference between Poincar\'e asymptotics and Gevrey asymptotics]}
...
Since different functions can be asymptotic to the same power series, taking the Borel sum of the asymptotic series of $\Phi$ must smooth away some details. We'll therefore call this process {\em Borel regularization}. We'll say $\Phi$ is {\em Borel-regularizable} if it's asymptotic to a Borel-summable power series (namely if we can draw the square in the lower part of the diagram). 

The natural question is then about the relationship between $\hat{\Phi}$ and $\Phi$: we say $\Phi$ is {\em Borel regular} if it is Borel-regularizable and Borel regularization leaves it unchanged, i.e. $\Phi$ is {\em Borel regular} if $\Phi\propto \hat{\Phi}$.
...
Using our new perspective on Borel summation, the central goal of this paper is to explain why it works for at least two kinds of problems: 
\begin{enumerate}
\item the first problem is solving a certain kind of ODE as \eqref{eqn:bessel-rescaled-1}. We will mainly focus on Poincar\'e rank $1$ ODEs
