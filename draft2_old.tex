...
%\end{equation}
%and $a_n\defeq (-1)^n\frac{1}{2\pi n!}\Gamma\left(n+\frac{5}{6}\right)\Gamma\left(n+\frac{1}{6}\right)\left(\frac{3}{4}\right)^n$. Analogously, let $\varphi(x)\defeq\sum_{n\geq 0}b_nx^{-n}$ be the asymptotic expansion of $Bi(x)$ as $x\to\infty$: from DLMF and Sauzin
%
%\begin{equation}
...
\begin{definition}
For $\alpha \in (0, 1)$, the fractional integral $\partial^{\alpha-1}_{x \text{ from } 0}$ is defined by
\[ \partial^{\alpha-1}_{x \text{ from } 0} f(x) \defeq \frac{1}{\Gamma(1-\alpha)} \int_0^x (x-x')^{-\alpha} f(x')\,dx'. \]
\end{definition}
Fractional derivatives $\partial^{n+\alpha}_{x \text{ from } 0}$ are defined for integers $n \ge -1$ by composing the fractional integral with powers of $\tfrac{\partial}{\partial x}$. However, $\partial^{\alpha-1}_{x \text{ from } 0}$ and $\tfrac{\partial}{\partial x}$ don't commute: their commutator is an initial value operator \textbf{[check, clarify]}. Various ordering conventions give various definitions of $\partial^{n+\alpha}_{x \text{ from } 0} f(x)$, which differ by operators that depend on the germ of $f$ at zero \textbf{[Lazarevi\'{c}, \S 1.3---original source Podlubny]}. We'll use the {\em Riemann-Liouville} convention.
\begin{definition}
For $\alpha \in (0, 1)$ and $n \ge -1$ integer, the {\em Riemann-Liouville fractional derivative} $\partial^{n+\alpha}_{x \text{ from } 0}$ is defined by
\[ \partial^{n+\alpha}_{x \text{ from } 0} \defeq \left(\tfrac{\partial}{\partial x}\right)^{n+1} \partial^{\alpha-1}_{x \text{ from } 0}. \]
\end{definition}
Each convention brings its own annoyances to interactions with the Borel transform.\footnote{See Remark~\ref{rmk:caputo} for more.} The Riemann-Liouville convention will be the least annoying for our purposes. Here's what's nice about it.
\begin{theorem}
Given a Gevrey-$1$ formal series $\tilde{g}(z) = \sum_{k \ge 0}a_kz^{-(k+1)}$, we have
\[ \partial^{n+\alpha}_{\zeta \text{ from } 0} \left[ \borel \tilde{g} \right](\zeta) = \left[ \borel z^{n+\alpha} \tilde{g} \right](\zeta) \]
...
\begin{align*}
\left[ \borel \tilde{I}_p \right](\zeta) & = \left[ \borel z^{1/2-1} \tilde{\varphi}_p \right](\zeta) \\
& = \partial^{1/2-1}_{\zeta \text{ from } 0} \left[ \borel \tilde{\varphi}_p \right](\zeta) \\
& = \partial^{1/2-1}_{\zeta \text{ from } 0} \hat{\varphi}_p(\zeta) \\
\end{align*}
\textbf{Oops, try again.}
By definition, $\tilde{\varphi}_p(z) = z^{1/2} \tilde{I}_p(z)$, so the theorem gives
\begin{align*}
\left[ \borel \tilde{\varphi}_p \right](\zeta) & = \left[ \borel z^{0+1/2} \tilde{I}_p \right](\zeta) \\
& = \partial^{0+1/2}_{\zeta \text{ from } 0} \left[ \borel \tilde{I}_p \right](\zeta) \\
\hat{\varphi}_p(\zeta) & = \tfrac{\partial}{\partial \zeta}\,\partial^{1/2-1}_{\zeta \text{ from } 0} \left[ \borel \tilde{I}_p \right](\zeta).
\end{align*}
Once we show that $\borel \tilde{I}_p = \hat{I}_p$ and
\[ \hat{I}_p(\zeta) = \tfrac{\partial}{\partial \zeta} \left( \int_{\mathcal{C}_p(\zeta)} \nu \right), \]
we have
\[ \hat{\varphi}_p(\zeta) = \tfrac{\partial}{\partial \zeta}\,\partial^{1/2-1}_{\zeta \text{ from } 0}\,\tfrac{\partial}{\partial \zeta} \left( \int_{\mathcal{C}_p(\zeta)} \nu \right). \]
\color{black}

\begin{lemma}
For $\alpha \in (0, 1)$ and $n \ge -1$,
\[ \partial^{n+\alpha}_{\zeta \text{ from } 0} \left[ \borel z^{-(k+1)} \right](\zeta) = \left[ \borel z^{n+\alpha} z^{-(k+1)} \right](\zeta) \]
\end{lemma}
\begin{proof}
First, evaluate
\begin{align*}
...
%%& = \left[ \borel z^{-[k-(\alpha-1)+1]} \right](\zeta) \\
%%& = \left[ \borel z^{\alpha-1} z^{-[k+1]} \right](\zeta)
\end{align*}
...
\end{align*}
Hence, \eqref{frac-diff-laplace} holds for all $n \ge -1$, and the desired result quickly follows. The condition $\alpha \in (0, 1)$ saves us from the trouble we'd run into if $k-(m+\alpha)$ were in $\Z_{\le 0}$. This is how we avoid the initial value corrections that appear in integer-order derivatives of Borel transforms.
\end{proof}
\color{MidnightBlue}
\begin{definition}
...
\begin{enumerate}
\item The series $\tilde{\varphi}_\alpha$ is Gevrey-1.
\item The series $\hat{\varphi}(\zeta)\defeq\mathcal{B}(\tilde{\varphi})$ converges near $\zeta=\zeta_{\alpha}$.
\item For any $\zeta$ on the ray going rightward from $\zeta_\alpha$, we have
...
\end{align*}
where $\mathcal{C}_\alpha(\zeta)$ is the part of $\mathcal{C}_\alpha$ that goes through $f^{-1}([\zeta_\alpha, \zeta])$. Notice that $\mathcal{C}_\alpha(\zeta)$ starts and ends in $f^{-1}(\zeta)$. \textbf{[The integral over $\mathcal{C}_\alpha(\zeta)$ needs to satisfy some condition near $\zeta = 0$ for this to hold. Figure out what that is! Also, be careful about the orientation of $\mathcal{C}_\alpha$.]}
\item
\end{enumerate}
\end{theorem}
\begin{proof}
Part~(1): Let's write $\approx$ when two functions are asymptotic (at all orders around the base point \textbf{[is this the right condition?]}), and $\sim$ when a function is asymptotic to a formal power series (at the truncation order of each partial sum).
...
\begin{equation}\label{formal-half-deriv}
\partial^{1/2}_{\zeta \text{ from }\zeta_\alpha}\mathcal{B}(\tilde{I}_{\alpha})(\zeta) = \hat{\varphi}_{\alpha}(\zeta).
\end{equation}
%%Let $\mathcal{C}^-_\alpha$ and $\mathcal{C}^+_\alpha$ be the parts of $\mathcal{C}_\alpha$ that go from the past to $x_\alpha$ and from $x_\alpha$ to the future, respectively. Noticing that $f \circ \mathcal{C}^\pm_\alpha$ are both injective, we can recast the integral $I_\alpha$ into the $f$ plane.
%%\[ I_{\alpha}(z) = \int_{\mathcal{C}^-_{\alpha}} e^{-zf} \nu + \int_{\mathcal{C}^+_{\alpha}} e^{-zf} \nu \]
Let's recast the integral $I_\alpha$ into the $f$ plane. As $\zeta$ goes rightward from $\zeta_\alpha$, the start and end points of $\mathcal{C}_\alpha(\zeta)$ sweep backward along $\mathcal{C}^-_\alpha(\zeta)$ and forward along $\mathcal{C}^+_\alpha(\zeta)$, respectively. Hence, we have
\begin{align*}
I_\alpha(z) & = \int_{\mathcal{C}_{\alpha}} e^{-zf} \nu \\
...
Observing that
\[ \partial_\zeta \left( \int_{\mathcal{C}_\alpha(\zeta)} \nu \right) = \left[\frac{\nu}{df}\right]_{\operatorname{start} \mathcal{C}_\alpha(\zeta)}^{\operatorname{end} \mathcal{C}_\alpha(\zeta)}, \]
we conclude that
...
\end{align*}
We already knew, from the general theory of the Borel transform, that the sum of $\mathcal{B}_{\zeta_\alpha}(\tilde{I}_\alpha)$ would be asymptotic to $\hat{I}_\alpha$. We've now shown that the sum of $\mathcal{B}_{\zeta_\alpha}(\tilde{I}_\alpha)$ is actually equal to $\hat{I}_\alpha$, strengthening the formal series identity~\ref{formal-half-deriv} to the analytic identity
\begin{align*}
\hat{\varphi}_{\alpha}(\zeta) & = \partial^{1/2}_{\zeta \text{ from } \zeta_\alpha} \hat{I}_\alpha \\
