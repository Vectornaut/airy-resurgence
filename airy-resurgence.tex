...
The coordinate $\xi$ given by $\zeta = m\xi$ defines a new translation structure on $\C$. The corresponding frequency variable $x \maps T^*\C \to \C$ is given by $d\xi \mapsto 1$, so we have $z = m^{-1} x$. From the calculation
\begin{align*}
\laplace_{\zeta, 0} \hat{\varphi} & = \int_{\Gamma_{\zeta, 0}} e^{-z \zeta}\,\hat{\varphi}\;d\zeta \\
& = \int_{\Gamma_{\xi, 0}} e^{-x\xi}\,\hat{\varphi}\;m\,d\xi \\
& = m \laplace_{\xi, 0} \hat{\varphi},
\end{align*}
we learn that
\[ \laplace_{\xi, 0} \hat{\varphi} = m^{-1} \laplace_{\zeta, 0} \hat{\varphi}. \]
