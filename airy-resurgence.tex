...
\usepackage{amsthm}
\usepackage{graphicx}
\usepackage{xcolor} %% for revisions
...
\subsubsection{Action on differential and integral operators}\label{L-diff-op}
...
\section{Shifting integro-differential equations}
...
Locally integrable slight functions play a special role in Laplace transform methods for linear differential equations. This is because differential equations in the spatial domain arise most naturally from integral equations in the frequency domain, but we'd like to work with differential equations in the frequency domain too. In the space of locally integrable slight functions, differential and integral equations enjoy their simplest equivalence.
...
\[ \fracderiv{-1}{\zeta}{0}\;\tfrac{\partial}{\partial \zeta}\;\phi = -\left[ \fracderiv{-1}{\zeta}{0} \circ h_0 + \sum_{k = 1}^m \fracderiv{-k}{\zeta}{0} \circ h_{-k-1} \right] \psi. \]
...
\[ \phi = -\left[ \sum_{k = 0}^m \fracderiv{-k}{\zeta}{0} \circ h_{-k-1} \right] \psi, \]
...
\[ \left[ z P\big(\tfrac{\partial}{\partial z}\big) + Q\big(\tfrac{\partial}{\partial z}\big) + R(z^{-1}) \right] \varphi = 0, \]
where $P$ is a polynomial, $Q$ is a polynomial of one degree lower, and $R$ is an entire function with no constant term. We'll recast equation~\ref{eqn:mod-bessel} in this form:
\begin{equation}\label{eqn:std-mod-bessel}
\left[z \big[ \big(\tfrac{\partial}{\partial z}\big)^2 - 1 \big] + \tfrac{\partial}{\partial z} - \big(\tfrac{1}{3}\big)^2 z^{-1} \right] \varphi = 0.
\end{equation}
...
\end{equation}
\color{black}
\subsection{Guessing solutions}
\subsection{Asymptotic analysis}
Equation~\ref{eqn:mod-bessel} has a regular singularity at $0$ and an irregular singularity at $\infty$. From the general theory of such equations, we know that the space of trans-series solutions has a basis of the form
