...
where $V_\alpha = \laplace_{\zeta_\alpha, 0} v_\alpha$ and $\zeta = \alpha + \zeta_\alpha$, and we learned in Section~\ref{reg-decay} that $V_\alpha$ is asymptotic to a scalar multiple of $z^{-1 - \tau_\alpha}$ at $z = \infty$. That means we can write our solution as
\[ e^{-\alpha z} z^{-\tau_\alpha} W_\alpha, \]
where $W_\alpha = z^{\tau_\alpha} V_\alpha$ is asymptotic to a scalar multiple of $z^{-1}$ at $z = \infty$.
\color{Peru}
\begin{align*}
...
\end{alignat*}
From the argument in Section~\ref{big-idea}, we know that $\laplace_{\zeta, 1} v_1$ satisfies equation~\ref{eqn:mod-bessel}, and can be written as $e^{-z} V_1$, where $V_1 = \laplace_{\zeta_1, 0} v_1$. Observing that $v_1$ has order $-1/2$, we learn that $V_1 = z^{1/2} W_1$ for some $W_1$ which is asymptotic to a scalar multiple of $z^{-1}$ at $z = \infty$.
