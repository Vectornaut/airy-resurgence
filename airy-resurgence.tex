...
\usepackage{tikz} %% for squiggly underlines

% TikZ libraries
\usetikzlibrary{decorations.pathmorphing} %% for squiggly underlines
...
\newcommand{\freqdom}{\undersquiggle{\C}}
\newcommand{\spacedom}{\dot{\C}}
\newcommand{\laplace}{\mathcal{L}}
\newcommand{\series}[1]{\tilde{#1}}
\newcommand{\fracderiv}[3]{\partial^{#1}_{#2, #3}}
\DeclareMathOperator{\Ai}{Ai}

% frougon's squiggly underlines
% https://tex.stackexchange.com/a/532011/6934
\makeatletter
...
\newdimen\@squiggle@yshift
\NewDocumentCommand{\undersquiggle}{m}{\mathord{\mathpalette\@undersquiggle{#1}}}

\newcommand*{\@undersquiggle}[2]{%
  \begin{tikzpicture}[baseline=(n.base)]
  \node (n) [inner sep=0]
    {\global\@squiggle@yshift=-0.5\math@param{#1}{2}{5}%
     $\m@th #1#2$};
    \draw[decorate, decoration={snake, amplitude=0.3pt, segment length=2pt}]
      ([yshift=\@squiggle@yshift]n.south west) -- ([yshift=\@squiggle@yshift]n.south east);
  \end{tikzpicture}%
}
\makeatother
...
Take two copies of the complex plane: the {\em frequency domain} $\freqdom$ and the {\em spatial domain} $\spacedom$, with standard coordinates $z$ and $\zeta$ respectively. The Laplace transform in $\zeta$ based at $\zeta = \alpha$ turns a function $f$ on $\zeta \in \alpha + (0, \infty)$ into a function $\laplace_{\zeta, \alpha} f$ on $z \in (0, \infty)$, defined by the integral
\[ \laplace_{\zeta, \alpha} f = \int_\alpha^{\alpha + \infty} e^{-z \zeta}\,f\;d\zeta. \]
For $\rho \in [0, \infty]$, recall that a function $f$ is in $O_{\zeta \to \rho}(g)$ if $|f| \lesssim g$ on some neighborhood\footnote{An open neighborhood in $\freqdom$ intersected with the domain of $f$.} of $\zeta = \rho$. A function is {\em subexponential} if it's in $O_{\zeta \to \infty}(e^{c\zeta})$ for all $c > 0$. Let $\mathcal{E}_{\zeta, \alpha}$ be the space of subexponential functions on $\zeta \in \alpha + (0, \infty)$ which are locally integrable throughout $\zeta \in \alpha + [0, \infty)$. If $f$ is in $\mathcal{E}_{\zeta, \alpha}$, then $\laplace_{\zeta, \alpha} f$ is well-defined, and it extends to a holomorphic function on $\Re(z) > 0$~\cite[\S 5.6]{diverg-resurg-i}.

For the Laplace transform based at $\zeta = 0$

The asymptotics of $f$ at $\zeta = \alpha$ control the asymptotics of $\laplace_{\zeta, \alpha} f$ at $z = \infty$. Once we see how this works for $\alpha = 0$, Section~\ref{translation} will do the rest. Let $F = \laplace_{\zeta, 0} f$. Equation~1.8 of \ref{laplace-tfm} shows\footnote{The argument cited still works in our generality. For holomorphic $f$, one can also use Equation 1.5 of {\em Borel-Laplace Transform and Asymptotic Theory} \textbf{(Sternin \& Shatalov)}.} that
\[ f \in O_{\zeta \to 0}(1) \quad\Longrightarrow\quad F \in O_{z \to \infty}\left(\frac{1}{z}\right). \]
More generally, for $\tau > -1$ \textbf{[prove or cite]},
\[ f \in O_{\zeta \to 0}(\zeta^\tau) \quad\Longrightarrow\quad F \in O_{z \to \infty}\left(\frac{1}{z^{1 + \tau}}\right). \]
Exact power law asymptotics relate similarly \textbf{[prove or cite]}:
\[ f \sim \zeta^\tau \text{ at } \zeta = 0 \quad\Longrightarrow\quad F \sim \frac{\Gamma(1+\tau)}{z^{1+\tau}} \text{ at } z = \infty. \]
...
In Sections \ref{pos-root}\,--\,\ref{neg-root}, we'll see this approach succeed. For each root $\alpha$, we'll find a solution $v_\alpha$ of equation~\ref{diff-eq:spatial-mod-bessel} which is slight and locally integrable at $\zeta = \alpha$. Calling its order $\tau_\alpha$, we'll normalize it so that $v_\alpha \sim \zeta^{\tau_\alpha}$ at $\zeta = \alpha$. We know $\laplace_{\zeta, \alpha} v_\alpha$ will satisfy equation~\ref{eqn:reg-mod-bessel}, and we can even work out its asymptotics. We know from Section~\ref{translation} that
\[ \laplace_{\zeta, \alpha} v_\alpha = e^{-\alpha z} V_\alpha, \]
where $V_\alpha = \laplace_{\zeta_\alpha, 0} v_\alpha$ and $\zeta = \alpha + \zeta_\alpha$, and we know from Section~\ref{reg-decay} that $V_\alpha \sim \Gamma(1 + \tau_\alpha)\,z^{-1 - \tau_\alpha}$ at $z = \infty$. Now we can write our solution as
\[ e^{-\alpha z} z^{-\tau_\alpha} W_\alpha, \]
where $W_\alpha = z^{\tau_\alpha} V_\alpha$ is asymptotic to $\Gamma(1 + \tau_\alpha)\,z^{-1}$ at $z = \infty$.
\color{Peru}
\begin{align*}
...
Let's find a solution of equation~\ref{diff-eq:spatial-mod-bessel} which is slight and locally integrable at $\zeta = 1$. Define a new coordinate $\zeta_1$ on $\spacedom$ so that $\zeta = 1 + \zeta_1$. In this coordinate, equation~\ref{diff-eq:spatial-mod-bessel} looks like
...
\end{alignat*}
From the argument in Section~\ref{big-idea}, we know that $\laplace_{\zeta, 1} v_1$ satisfies equation~\ref{eqn:mod-bessel}, and can be written as $e^{-z} V_1$, where $V_1 = \laplace_{\zeta_1, 0} v_1$. Observing that $v_1$ has order $-1/2$, we learn that $V_1 = z^{1/2} W_1$ with $W_1 \sim \Gamma\big(\tfrac{1}{2}\big)\,z^{-1}$ at $z = \infty$.
