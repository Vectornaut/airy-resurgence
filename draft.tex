...
\begin{equation}
\left[\frac{\partial^2}{\partial z^2}+2\frac{\partial}{\partial z}+\frac{1}{z}\frac{\partial}{\partial z}-\frac{1}{9z^2}\right]\kappa=0
\end{equation}
Its Borel transform is 
\begin{align*}
&\zeta^2\hat{\kappa}-2\zeta\hat{\kappa}+1\ast(-\zeta\hat{\kappa})-\frac{1}{9}\zeta\ast\hat{\kappa}=0 \\
&\zeta^2\hat{\kappa}-2\zeta\hat{\kappa}-\int_{0}^\zeta s\hat{\kappa}(s)ds-\frac{1}{9}\int_0^{\zeta}(\zeta-s)\hat{\kappa}(s)ds=0 \\
&\, \text{ taking derivatives once} \\
& 2\zeta\hat{\kappa}+\zeta^2\hat{\kappa}'-2\hat{\kappa}-2\zeta\hat{\kappa}'-\zeta\hat{\kappa}-\frac{1}{9}\int_0^{\zeta}\hat{k}(s)ds=0\\
& \zeta\hat{\kappa}+\zeta^2\hat{\kappa}'-2\hat{\kappa}-2\zeta\hat{\kappa}'-\frac{1}{9}\int_0^{\zeta}\hat{k}(s)ds=0\\
&\, \text{ taking derivatives once again} \\
& \hat{\kappa}+\zeta\hat{\kappa}'+2\zeta\hat{k}'+\zeta^2\hat{\kappa}''-2\hat{\kappa}'-2\hat{\kappa}'-2\zeta\hat{\kappa}''-\frac{1}{9}\hat{\kappa}=0\\
&(\zeta^2-2\zeta)\hat{\kappa}''+(3\zeta-4)\hat{k}'+\frac{8}{9}\hat{\kappa}=0
\end{align*} 
The last equation is a hypergeomtric equation and a solution is given by 
\begin{equation}
\hat{\kappa}(\zeta)={}_2F_1\left(\frac{2}{3},\frac{4}{3};2;\frac{\zeta}{2}\right)
\end{equation}
