...
\end{definition}
Each convention brings its own annoyances to interactions with the Borel transform.\footnote{See Remark~\ref{rmk:caputo} for more.} The Riemann-Liouville convention will be the least annoying for our purposes.
\begin{theorem}
Take a Gevrey-$1$ formal series $\varphi(z) = \sum_{k \ge 0}a_kz^{-(k+1)}$. For $\alpha \in (0, 1)$ and $n \ge -1$, we have
\[ \partial^{n+\alpha}_{\zeta \text{ from } 0} \left[ \borel \varphi \right](\zeta) = \left[ \borel z^{n+\alpha} \varphi \right](\zeta) \]
\end{theorem}
...
& = \frac{\zeta^{k-(\alpha-1)}}{\Gamma(k-\alpha)}
