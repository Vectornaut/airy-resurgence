...
\begin{itemize}
\item[(i)] \emph{[fractional derivative]} if $\alpha\in\R\setminus\Z_{\leq 0}$, $\mathcal{B}\left(z^\alpha f(z)\right)=\partial_{\zeta}^{\alpha}\hat{f}(\zeta)$
\item[(ii)] \emph{[fractional integral]} if $\alpha\in\R\setminus\Z_{\geq 0}$, $\mathcal{B}\left(z^\alpha f(z)\right)=\partial_{\zeta}^{\alpha}\hat{f}(\zeta)$
...
\end{align*}
These are evidence of the resurgent properties of $\tilde{I}_{\pm 1}(z)$. \textbf{Hopefully, the latter identities show that the Stokes constants can be computed via Alien calculus and they are equal to $\pm 2$.  } 
...
\xi^{1/2}{}_2F_1\left(\frac{1}{2},\frac{1}{2};\frac{3}{2};-\xi\right)&=i\frac{\theta}{2}\\
 &=\frac{i}{2}\left(\frac{1}{2}\mathrm{arcosh}\left(\frac{\zeta}{2}\right)\right)\\
...
&=-\frac{i\sqrt{\pi}}{16} \hat{w}_+(\zeta-2) &
\end{align*}
