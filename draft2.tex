...
\color{black}
We breifly recall some basic facts on Borel transform and its properties (more details can be founded at \cite{MS}, \cite{MDiablerets}, \cite{}, \cite{}). The Borel transform $\mathcal{B}$ is a linear map from formal power seires in the time domain $\C_z$ to the space domain $\C_{\zeta}$: \begin{align*}\mathcal{B}(z^{-n-1})&\defeq\tfrac{\zeta^n}{n!} \qquad, \text{ if } n\geq 0 \\
\mathcal{B}(1)&\defeq\delta 
\end{align*} 

where $\delta$ is the element $(1,0)\in\C\times\C[\![\zeta]\!]$\footnote{In the physical literature (\cite{MDiablerets}\cite{SGM}) many authors adopt a different convention, namely they define $\mathbb{B}(z^{-n})\defeq \tfrac{\zeta^n}{n!}$. However, we find more natural the mathematical convention as the Laplace transform is the inverse of $\mathcal{B}$ under suitable growth conditions. }, and $\mathcal{B}$ extends formally by linearity to  
...
Notice that we are at the level of formal series, so there are no convergence assumptions. However, there is a special class of formal series that behaves well under Borel transform, meaning that its Borel transform gives a holomorphic fucntion at the origin in $\C_{\zeta}$. These are the so called Gevrey-$1$ series: $f(z)=\sum_{n\geq 0}a_nz^{-n}$ is Gevrey-$1$ if there exists $A>0$ such that $|a_n|\leq A^n n!$ for all $n\geq 0$.\footnote{In asymptotic analysis Gevrey-$k$ series $\sum_{n\geq 0}a_nz^{-n}$ have coefficients that grow as $(n!)^{k}$, i.e. there exists $A>0$ such that $|a_n|\leq A^n (n!)^k$ for every $n\geq 0$. }    

\begin{lemma}
Let $f(z)=\sum_{n\geq 0}a_nz^{-n}$, $f(z)$ is Gevrey-$1$ if and only if $\hat{f}(\zeta)\defeq\mathcal{B}(f)\in\C\lbrace\zeta\rbrace$.  
\end{lemma}
...
However, if $f(z)=\sum_{n\geq 0}a_nz^{-n}$ is a Gevrey-$1$ series, then the Borel transform of $\mathcal{B}(z^{-\alpha}f)(\zeta)$ is a germ of meromorphic function at the oringin: indeed 
\begin{align*}
\mathcal{B}(z^{\alpha}f)(\zeta)&=\frac{\zeta^{\alpha-1}}{\Gamma(\alpha)}\ast\left(a_0\delta+\sum_{n\geq 0}a_{n+1}\frac{\zeta^n}{n!}\right)\\
&=a_0\frac{\zeta^{\alpha-1}}{\Gamma(\alpha)}+\sum_{n\geq 0}a_{n+1}\frac{\zeta^{n+\alpha}}{\Gamma(1+n+\alpha)}
\end{align*}

In particular, if $\alpha\in (1,+\infty)$, then $\mathcal{B}(z^{\alpha}f)(\zeta)$ is actually a germ of holomorphic functions on the Riemann surface of $\zeta
^{\alpha-1}$, otherwise if $\alpha\in (-\infty,1)$ then $\mathcal{B}(z^{\alpha}f)(\zeta)$ is a meromorphic functions on the Riemann surface where $\zeta^{\alpha-1}$ defines an analytic coordinate. The case of interest for usa in the following is when $\alpha=\tfrac{1}{2}$, hence if $a_0\neq 0$, the Borel trasform will be merorphic at the origin and definied on the Riemann surface of the square root.     

\subsubsection{Properties of the Borel transform}
We now recall some properties of the Borel transform: let $f(z)=\sum_{n\geq 0}a_nz^{-n}$ and denote by $\hat{f}(\zeta)\defeq\mathcal{B}(f(z))$

\begin{itemize}
\item[(i)] \emph{[fractional derivative]} if $\alpha\in\R_{\geq 0}$, $\mathcal{B}\left(z^\alpha f(z)\right)=\partial_{\zeta}^{\alpha}\hat{f}(\zeta)$
\item[(ii)] \emph{[fractional integral]} if $\alpha\in\R_{<0}\setminus\Z_{\geq 0}$, $\mathcal{B}\left(z^\alpha f(z)\right)=\partial_{\zeta}^{\alpha}\hat{f}(\zeta)$
\item[(iii)] if $n\in\Z$ and $z^nf(z)\in z^{-1}\C[\![z^{-1}]\!]$, then $\mathcal{B}(z^nf(z))=\partial_\zeta^n\hat{f}(\zeta)$ 
\item[(iv)] $\mathcal{B}\left(\partial_z^{n} f(z)\right)=(-\zeta)^n\hat{f}(\zeta)$, for every $n\geq 0$
\item[(v)] $\mathcal{B}(f(z-c))=e^{-c\zeta}\hat{f}(\zeta)$
\item[(vi)] if $g(z)\in z^{-1}\C[\![z^{-1}]\!]$, then $\mathcal{B}(f(z)g(z))=\int_0^{\zeta}d\zeta' \hat{f}(\zeta-\zeta')\hat{g}(\zeta')=:\hat{f}\ast \hat{g}$, where $\hat{g}(\zeta)=\mathcal{B}(g(z))$.
\end{itemize} 
...
(ii) Notice that for $\alpha\in\R\setminus\Z_{\geq 0}$ the fractional integral $\partial_\zeta^{\alpha}\zeta^{k}=\zeta^{k-\alpha}\tfrac{k!}{\Gamma(k-\alpha+1)}$, hence \[\borel (z^\alpha f(z))=\sum_{k\geq 0}a_k\tfrac{\zeta^{k-\alpha}}{\Gamma(k-\alpha+1)}=\sum_{k\geq 0}a_k\tfrac{\zeta^{k-\alpha}}{k!}\tfrac{k!}{\Gamma(k-\alpha+1)}=\sum_{k\geq 0}a_k \tfrac{1}{k!}\partial_\zeta^{\alpha}\zeta^k=\partial_\zeta^\alpha\hat{f}(\zeta).\]

(iii) $z^nf(z)=\sum_{k\geq 0}a_kz^{-(k-n)-1} $ and by assumption $k-n\geq 0$, hence by defintion \[\borel (z^n f(z))=\sum_{k\geq n} a_k\tfrac{\zeta^{k-n}}{(k-n)!}=\sum_{k\geq n} a_k\tfrac{1}{k!}\tfrac{k!\zeta^{k-n}}{(k-n)!}=\sum_{k\geq n} a_k\tfrac{1}{k!}\partial_\zeta^n\zeta^k=\partial_\zeta^n\hat{f}(\zeta).\] 

(vi) $\partial_z^n f(z)=\sum_{k\geq 0}a_k(-1)^{n}z^{-k-n-1}\tfrac{\Gamma(k+n+1)}{k!}$, hence \[\borel (z^\alpha f(z))=\sum_{k\geq 0}a_k(-1)^{n}\tfrac{\zeta^{k+n}}{\Gamma(k+n+1)}\tfrac{\Gamma(k+n+1)}{k!}=(-\zeta)^{n}\hat{f}(\zeta).\]
...
\begin{align*}
\borel (z^\alpha f(z))&=\borel (z^\alpha)\ast \hat{f}(\zeta)\\
&=\frac{\zeta^{-\alpha-1}}{\Gamma(-\alpha)}\ast\hat{f}(\zeta)\\
...
&=\partial_\zeta^\alpha\hat{f}(\zeta)
\end{align*}
\end{remark}

\subsection{Laplace transform}
 \textbf{Copy from Aaron}
 

\subsection{Borel--Laplace summability}
...
Alternative to Borel--Laplace summability, J. Ecalle introduced the theory of resurgence: divergent power series that are Gevrey-$1$ become germs of holomorphic functions at the origin in the Borel plane. Being divergent, they have singularities in Borel plane and the aim is to investigate the type of singularities and the analytic continuation of the germ. Indeed a formal series is resurgent if it admits an \textit{endless} analytic continuation.
...
\begin{align}
\label{w+}&\hat{w}_+(\zeta)=c_1\,\, {}_2F_1\left(\frac{3}{2},\frac{3}{2};2;-\frac{\zeta}{4}\right)\\
\label{w+}&\hat{w}_-(\zeta)=c_2\,\, {}_2F_1\left(\frac{3}{2},\frac{3}{2};2;\frac{\zeta}{4}\right)
\end{align}
